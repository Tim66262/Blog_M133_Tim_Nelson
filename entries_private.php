<?php
  ...
  if ($killC) {
	...
	if (deleteComment($cId)) {
	  $eId = 0;
	  $meldung = "<div class='alert alert-success'><p>Kommentar wurde erfolgreich gelöscht.</p></div>";
	} else {
	  $meldung = "<div class='alert alert-danger'>Kommentar konnte nicht gelöscht werden.</div>";
	}
  }
	if ($killE) {
	if (deleteEntry($eId)) {
  ...
  <div class="col-md-4">
	<?php
	  ...
	  foreach($entries as $entry) {
		...
		echo "<a class='list-group-item$active' href='index.php?function=entries_private&eid=$entry[0]&bid=$uid' title='Beitrag anzeigen'>";
  ...
  <?php
	...
	if ($eId > 0) {
	  ...
	  if (is_array($entry)) {
		...
		echo "<a href='index.php?function=entries_private&killE=true&eid=$entry[0]' title='Beitrag löschen' ><span class='glyphicon glyphicon-remove'></span></a>";
		echo "</div>";
		...
		foreach($comments as $comment) {
		  ...
		  $cId = $comment[0];
		  echo "<p>";
		  echo "<a href='index.php?function=entries_private&killC=true&cid=$cId&eid=$eId&bid=$uId' title='Beitrag löschen''><span class='glyphicon glyphicon-remove'></span></a>";
	...
	echo "<form name='formular' method='post' action='".$_SERVER['PHP_SELF']."?function=entries_private&eid=$entry[0]&bid=$uId'>";
